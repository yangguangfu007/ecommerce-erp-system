  ...
  <div class="data-sync-panel">
    ...
    <div class="sync-operations">
      <div class="operation-header">
        <h3 class="operation-title">数据同步操作</h3>
        ...
        <div class="sync-type-grid">
          ...
          >
            ...
            <div v-if="syncType.status" class="sync-status">
              <el-tag :type="getSyncStatusType(syncType.status)">
...
-- 同步进度显示 -->
    <div v-if="activeTasks.length > 0" class="sync-progress-section">
      <div class="progress-header">
        <h3 class="progress-title">同步进度</h3>
        ...
        >
          <div class="progress-info">
            ...
            <div class="task-details">
              ...
              <div v-if="task.endTime" class="detail-item">
                <span class="detail-label">结束时间:</span>
                <span class="detail-value">{{ formatDateTime(task.endTime) }}</span>
              </div>
              <div class="detail-item">
...
- 同步日志查看 -->
   ...
   <!-- 同步计划设置 -->
    <div class="sync-schedule-section">
      <div class="schedule-header">
        <h3 class="schedule-title">同步计划设置</h3>
...
import { 
  ...
  Box, ShoppingCart, House, Setting
} from '@element-plus/icons-vue'
import { platformApi } from '@/api/modules/platform'
import type { SyncDataResult, SyncLog, SyncLogQuery } from '@/api/modules/platform'
...
// 计算属性
const hasActiveTasksComputed = computed(() => {
...
const calculateNextExecutionTime = () => {
  ...
  switch (scheduleForm.frequency) {
    case 'hourly':
      next.setHours(next.getHours() + scheduleForm.intervalHours)
      break
    case 'daily':
      if (scheduleForm.dailyTime) {
        next.setHours(scheduleForm.dailyTime.getHours(), scheduleForm.dailyTime.getMinutes(), 0, 0)
        if (next <= now) {
  ...
  nextExecutionTime.value = next.toISOString()
}
...
const getTaskDisplayName = (task: SyncDataResult) => {
  ...
  return typeMap[task.syncType] || `${task.syncType}同步`
}
...
onMounted(() => {
  ...
  // 如果有活跃任务，开始轮询
  if (hasActiveTasksComputed.value) {
    startStatusPolling()
  }
})
